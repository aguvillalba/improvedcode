...
function ImprovedCodeEditor() {
    ...
    this.init = function () {
        ...
        _cmSettings.cme.setOption("extraKeys", {
            Tab: function(cm) {
    ...
    function _indentCode(on) {
        ...
        var preTagOpen = false;
        var preTagIndentBlocks = 0;
        for(var i=0;i<lines;i++) {
            var lineContent = _cmSettings.cme.getLine(i);
            /* If there is any <pre> tag, we don't follow regular indent,
             * instead, we reset the indentation and start from very left and keep increasing when necessary, so we
             * avoid CodeMirror to indent anything inside <pre> tags as if it was normal code.
             */
            if (!preTagOpen) {
                preTagOpen = (lineContent.search('<pre>') != -1);
            } else {
                preTagOpen = (lineContent.search('</pre>') == -1);
                preTagIndentBlocks = 0;
            }

            if (preTagOpen) { //If <pre> tag, don't follow regular indentation
                _cmSettings.cme.indentLine(i,preTagIndentBlocks*_cmSettings.cme.getOption('indentUnit'));
                preTagIndentBlocks++;
            } else {
